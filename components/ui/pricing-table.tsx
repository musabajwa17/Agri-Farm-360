...
export function PricingTable({
  ...
  return (
    ...
    >
      <div
        className={cn("max-w-4xl mx-auto px-4", containerClassName)}
        {...props}
      >
            ...
            <button
              ...
              className={cn(
                ...
                selectedPlan === plan.level &&
                  "ring-2 ring-[#39ac63] dark:ring-[#39ac63]"
              )}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">{plan.name}</span>
                {plan.popular && (
                  <span className="text-xs bg-[#39ac63] dark:bg-[#39ac63] text-[#a2ffc4] dark:text-[#a2ffc4] px-2 py-0.5 rounded-full">
                    Popular
                  </span>
                )}
              </div>
                    ...
                    {plans.map((plan) => (
                      ...
                      >
                        {shouldShowCheck(feature.included, plan.level) ? (
                          <CheckIcon className="w-5 h-5 text-[#39ac63]" />
                        ) : (
