  ...
  const plans = [
    {
      ...
      description: "Perfect for getting started",
      price: "29",
      features: ["Access to core features", "Email support included", "Up to 5 team members"],
      gradient: "from-emerald-400 to-teal-500",
      iconColor: "text-emerald-400",
      highlight: false,
    },
    {
      ...
      description: "Most popular for growing teams",
      price: "79",
      features: ["Everything in Basic plus", "Priority support 24/7", "Unlimited team members"],
      gradient: "from-violet-400 to-fuchsia-500",
      iconColor: "text-violet-400",
      highlight: true,
    },
    {
      ...
      description: "Enterprise-grade solution",
      price: "149",
      features: ["Everything in Premium plus", "Dedicated account manager", "Custom integrations & API"],
      gradient: "from-amber-400 to-orange-500",
      iconColor: "text-amber-400",
      highlight: false,
    },
      ...
      <div className="max-w-7xl mx-auto py-20 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-6">
          {plans.map((plan) => {
            const Icon = plan.icon;
            return (
              <div key={plan.name} className={`relative group ${plan.highlight ? "lg:-mt-4 lg:mb-4" : ""}`}>
                {/* Popular Badge */}
                {plan.highlight && (
                  <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 z-10">
                    <div className="relative">
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    {/* Plan Name */}
                    <h3 className={`text-4xl font-extrabold mb-3 ${isDark ? "text-white" : "text-gray-900"}`}>{plan.name}</h3>

                    {/* Description */}
                    <p className={`text-base mb-8 leading-relaxed ${isDark ? "text-gray-400" : "text-gray-600"}`}>{plan.description}</p>

                    {/* Price */}
                    <div className="mb-10">
                      <div className="flex items-baseline gap-1">
                        <span className={`text-6xl font-black tracking-tight ${plan.iconColor}`}>${plan.price}</span>
                        <span className="text-xl font-medium text-gray-500">/mo</span>
                      </div>
                      <p className="text-sm mt-2 font-medium text-gray-500">30 days free trial</p>
                    </div>
                    ...
                    <div className="flex-grow mb-10 space-y-5">
                      {plan.features.map((feature, idx) => (
                        <div key={idx} className="flex items-start gap-4 group/item">
                          ...
                          <span className={`text-base leading-relaxed ${isDark ? "text-gray-300" : "text-gray-700"}`}>{feature}</span>
                        </div>
                      ))}
                    </div>
        ...
        {/* Bottom Text */}
        <div className="text-center mt-20">
          <div className={`inline-flex items-center gap-2 px-6 py-3 rounded-full ${isDark ? "bg-gray-800/50 border border-gray-700/50" : "bg-gray-100 border border-gray-200"}`}>
            <div className="flex items-center gap-4 text-sm">
              <span className={`flex items-center gap-2 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                <Check className="w-4 h-4 text-emerald-500" strokeWidth={3} /> 30-day free trial
              </span>
              <span className={`${isDark ? "text-gray-700" : "text-gray-300"}`}>•</span>
              <span className={`flex items-center gap-2 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                <Check className="w-4 h-4 text-emerald-500" strokeWidth={3} /> No credit card required
              </span>
              <span className={`${isDark ? "text-gray-700" : "text-gray-300"}`}>•</span>
              <span className={`flex items-center gap-2 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                <Check className="w-4 h-4 text-emerald-500" strokeWidth={3} /> Cancel anytime
              </span>
            </div>
          </div>
        </div>
